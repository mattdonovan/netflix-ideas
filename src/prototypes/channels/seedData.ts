 ...
 *
 * Games are pulled out of the fresh block and inserted directly after Top 10.
 * The 2025 Netflix TV redesign gave games their own nav slot and the same
 * card, badges, and detail page as film and television — the mixed-media
 * premise this prototype is built on, shipped by Netflix itself. Leaving the
 * games row at the bottom of the stack would contradict the thing we're
 * quoting.
 */
const GAMES_ID = "netflix-games";
const PROMOTE_AFTER = "top-10-tv-us";

function orderChannels(base: Channel[], fresh: Channel[]): Channel[] {
  const games = fresh.find((c) => c.id === GAMES_ID);
  const rest = [...base, ...fresh.filter((c) => c.id !== GAMES_ID)];
  if (!games) return rest;
  const at = rest.findIndex((c) => c.id === PROMOTE_AFTER);
  if (at < 0) return [...rest, games];
  return [...rest.slice(0, at + 1), games, ...rest.slice(at + 1)];
}

export const seedChannels: Channel[] = orderChannels(baseChannels, freshChannels);
